...
const ProductCard = ({ image, title, text, price, button }) => {
  return (
    <div className="w-full max-w-80 mx-auto flex flex-col items-center bg-white  rounded-md mb-4">
      <img className="w-80 rounded-md shadow-md" src={image} alt={title} />
      ...
      <div className="flex items-center w-full mb-4 gap-2 py-2">
        <button className="flex items-center justify-center gap-2 flex-row flex-grow font-belleza border bg-black text-white px-4 py-2 rounded-md tracking-wide  shadow-md  hover:tracking-wider transform transition-all duration-300">
          ADD TO CART <BsCart />
          {button}
        </button>
      </div>
